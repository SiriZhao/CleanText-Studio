...
`ProviderPreset` 统一 Provider 默认地址和模型建议；`ParagraphLayoutEngine` 根据 TextBlock 连接正文；`SystemThemeWatcher` 低频读取可替换的系统主题 reader；`FontManager` 只选择系统已安装字体；所有控件颜色、圆角和间距由 DesignToken 集中生成。v1.2.0 的表格解析器将连续 Markdown 表格行聚合为 TableData，DocxRenderer 将其输出为原生 Word 表格。
